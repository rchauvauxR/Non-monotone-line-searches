function z = perf_profile(H,gate,logplot,dic)
...
for s = 1:ns
    ...
    if (logplot)
        hl(s) = semilogx(xs,ys,option1,'MarkerSize',5,'MarkerIndices',1:1000:length(xs),'LineWidth',1);
    else
        hl(s) = plot(xs,ys,option1,'MarkerSize',5,'MarkerIndices',1:1000:length(xs),'LineWidth',1);
    end
    hold on;
end
...
title( "Performance profile with $\tau$ = "+double(gate),'Interpreter','latex' );
xlabel("Performance ratio alpha",'Interpreter','latex');
legend(dic,'Location','northwest');
